...
本 Skill 可作为 `doc-format` 文件夹独立安装；`scripts` 目录内包含完整的 CLI、核心排版逻辑、默认配置和内置测试代码。维护 GUI 版本后，如核心逻辑有变更，应同步复制 `wfp_cli.py`、`wfp_core.py`、`wfp_config.py`、`wfp_tests.py` 到本 Skill 的 `scripts` 目录。

...
*   **多格式输入**：支持 `.docx`，并可将 `.doc`、`.wps`、`.txt`、`.md` 转换为 `.docx` 后处理。`.doc/.wps` 转换依赖 Windows、WPS/Word 和 pywin32；非 Windows 环境可处理 `.docx/.txt/.md`，遇到 `.doc/.wps` 时会给出明确提示。
...
1.  **环境依赖**：
    ...
    *   非 Windows 环境可处理 `.docx/.txt/.md`，但无法使用 WPS/Word COM 预处理。
...
安装后，Agent 可按 Skill 说明调用 `scripts/wfp_cli.py`。CLI 支持 JSON 配置：默认读取当前目录的 `wfp_config.json`，也可通过 `--config` 指定配置文件；可使用 `show-config` 查看当前配置，使用 `save-config` 保存修改后的配置。配置完成后，还可按需启用表格内容自动调整、自定义数字和字母字体（默认 Times New Roman）以及符号标准化，再重新执行一次格式化。

跨平台转换说明：Windows 下处理 `.doc/.wps` 会优先尝试 WPS 或 Microsoft Word 接口，并尝试将自动编号转为可编辑文本；macOS/Linux 下处理 `.doc/.wps` 会尝试使用 LibreOffice 转为 `.docx`。如果转换失败，请先将文档手动另存为 `.docx` 后再输入。若自动编号未能转换，处理完成后需人工检查自动编号文本、段落格式是否正常。
...
A：请参考“方式二：从源码运行”，在本机配置 Python 环境并安装依赖。国产操作系统或其他非 Windows 环境可处理 `.docx/.txt/.md`；如果需要处理 `.doc/.wps`，建议先手动另存为 `.docx`，再输入本工具。

**Q4：点击“开始排版”后程序报错、卡住或无响应？**

A：v2.7.1 已将排版任务放入后台线程，并增加进度条和状态提示。如果仍然报错，请查看日志窗口中的错误信息。处理 `.doc/.wps`、修订或自动编号转换时，请确认电脑已正确安装 **Microsoft Office** 或 **WPS Office**，并具备 pywin32 环境；若程序结束后无法打开文档，可以在任务管理器中结束相关 Word/WPS 进程后重新打开。
...
A：当前 GUI 版本的自动编号转文本依赖 WPS/Word COM，因此非 Windows 环境会跳过该步骤。处理完成后请人工检查自动编号，尤其是无法单独选中的编号，其字体字号可能仍由编号样式控制。

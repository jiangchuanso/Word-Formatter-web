...
2.  **运行程序**：Windows 下双击 `.exe` 即可使用；macOS 下解压 `.zip` 后双击 `.app`，未签名首次运行如遇系统拦截，可在“系统设置 > 隐私与安全性”中允许打开；Kylin/Linux 下运行发布包中的 `.AppImage`。如果系统未自动赋予执行权限，可在程序目录执行：
    ```bash
    chmod +x Word-Formatter-Pro.v2.7.5.Kylin-V10.x86_64.AppImage
    ./Word-Formatter-Pro.v2.7.5.Kylin-V10.x86_64.AppImage
    ```
...
```bash
# macOS Apple Silicon：生成 release/Word-Formatter-Pro.v2.7.5.macOS-arm64.app.zip
python3 packaging/build_release.py macos --arch arm64

# Windows：生成 release/Word-Formatter-Pro.v2.7.5.exe
python packaging/build_release.py windows

# Kylin/Linux：生成 release/Word-Formatter-Pro.v2.7.5.Kylin-V10.x86_64.AppImage
python packaging/build_release.py kylin --arch x86_64 --appimagetool /path/to/appimagetool

# v2.7.5 发布时复用 v2.7.4 Windows/Kylin 二进制并重命名
